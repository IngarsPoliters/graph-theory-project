...
class State:
    ...
    def __init__(self, label, arrows, accept):
        """label is the arrow labels, arrows is a list of states to
            pont to, accept is a boolean as to whether this is an accept state"""
        self.label = label
        self.arrows = arrows
        self.accept = accept
...
def re_to_nfa(postfix):
    # A stack for NFAs.
    stack =[]
    # Loop through the postifx r.e. left to right.
    for c in postfix:
        # Concatenation.
        if c == '.':
            ...
            # Point old end tates to new one.
            ...
            nfa1.end.arrows.append(nfa.start)
            # Make a new NFA.
            nfa = NFA(start, end)
            # Push to the stack.
            stack.append(nfa)
            ...
            # Create an NFA for a the non-special character c.
